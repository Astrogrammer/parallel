...
6. [Часто задаваемые вопросы](#questions)
...
**Внимание!** Поддерживаются только версии не выше 1.85.2.  
Установка двух версий одновременно описана [в отдельном гайде](https://github.com/ded-otshelnik/parallel-programming/blob/main/practices/vscode-remote-linux.md#установка-vscode-подходящей-под-суперкомпьютер).
...
*Вариант 2:* ключ сохранён не в `/home/user/.ssh` или `C:\Users\user\.ssh`  
**Решение 1:** перенести приватный ключ в стандартный каталог.  
**Решение 2:** с помощью ключа -i указать расположение ключа:  
```
ssh ivanov_ii@hpc.cs.vsu.ru -i /home/ivanov_ii/keys/private
```  

*Вариант 3:* ключ был сгенерирован через PuTTY и не запущен pageant.  
...
*Вариант 4:* ключ сгенерирован на другом компьютере или на другой операционной системе.  
...
*Вариант 5:* ключ сгенерирован через PuTTY, а вход осуществляется через терминал или VS Code.  
**Решение 1:** войдите через PuTTY, не забыв запустить pageant.  
**Решение 2:** сгенерируйте новую пару ключей через `ssh keygen -t rsa`  
и отправьте публичный ключ администратору.  

*Вариант 6:* ключ был сгенерирован на MacOS через Wi-Fi ВГУ или ФКН.  
...
**Ошибка:** slurmstepd: error: execve(): /opt/slurm-wrappers/gcc: No such file or directory  
**Причина:** отличие bash-окружения на VS Code от терминального.  
**Решение:** введите `ssh head` в консоли VS Code и снова запустите gcc.  
Обратите внимание, что переход между папками нужно будет делать через `cd`.  

### <a id="compiling_programs">Можно ли на кластере компилировать программы?</a>
...
### <a id="without_slurm">Можно ли запускать программы вне очереди исполнения?</a>
...
### <a id="break">Могу ли я случайно что-нибудь сломать на кластере?</a>
...
### <a id="making_programs">Могу ли я самостоятельно устанавливать программы на кластер?</a>
Да, но только для себя. Установка программ для всех пользователей требует наличия рут-прав, которые студентам и преподавателям не предоставляются. По этой причине команды установки программ из репозиториев вида `sudo zypper install`, `sudo yum install` для вас недоступны.  
В то же время, если вы не нашли нужного ПО в установленных [модулях](#modules), вы всегда можете собрать его сами из исходных кодов. Для этого:  
1. Перейдите на узел хранилища через `ssh lustre`. На нём установлена ОС Linux CentOS 7, аналогичная ОС на вычислительных узлах, что делает сборку на нём более предпочтительной для обеспечения совместимости с ними. Имейте в виду, что на узле хранилища папка `/home` превращается в `/lustre/home`.
2. Создайте папку для хранения исходных кодов (к примеру, `/lustre/home/ivanov_ii/tar`).
3. Создайте папку для хранения исполняемых файлов программ (к примеру, `/lustre/home/ivanov_ii/bin`).
4. Перейдите в папку `tar` и скачайте при помощи команды `wget` архив с программой в формате `.tar.gz` или аналогичном.
5. Распакуйте архив при помощи команды `tar xvfz` или `tar xvf`.
6. Перейдите в папку с исходными кодами программы и далее следуйте инструкциям по установке данной конкретной программы. Они могут содержаться в `README`, `INSTALL` или на официальном сайте. В общем виде, установка выглядит следующим образом.
7. Исполните файл `./autogen.sh`, если такой присутствует. Если его нет, выполните `autoreconf -i` для перенстройки конфигурационных файлов под текущую ОС. Если команда не срабатывает, этот шаг можно пропустить.
8. Сконфигурируйте программу. Для этого сначала выполните `./configure -h` для отображения полного списка доступных ключей. Выберите те из них, которые кажутся вам наиболее подходящими. Обязателен к использованию `--prefix=/lustre/home/ivanov_ii/...`, который обеспечит установку программы в вашу личную папку. Также полезными часто бывают `--enable-shared` и `--enable-optimizations`. Выполните команду `./configure --prefix=... <прочие ключи>`
9. Соберите программу командой `make`. Проверьте корректность сборки через `make check`, если это возможно.
10. Выполните установку через `make install`. Если правильно указан prefix, рут-права не потребуются.
11. Пропишите переменные окружения в файле `/lustre/home/ivanov_ii/.bashrc` по следующему образцу:
    ```
    PATH=/lustre/home/ivanov_ii/bin/python/3.11/bin:$PATH  
    C_INCLUDE_PATH=/lustre/home/ivanov_ii/bin/python/3.11/include:$C_INCLUDE_PATH  
    LIBRARY_PATH=/lustre/home/ivanov_ii/bin/python/3.11/lib:$LIBRARY_PATH  
    LD_LIBRARY_PATH=/lustre/home/ivanov_ii/bin/python/3.11/lib:$LD_LIBRARY_PATH  
    LD_RUN_PATH=/lustre/home/ivanov_ii/bin/python/3.11/lib:$LD_RUN_PATH  
    MANPATH=/lustre/home/ivanov_ii/bin/python/3.11/share/man:$MANPATH
    ```
    **Переменные и нужные расположения могут отличаться, приведён лишь один из распространённых наборов.**
12. Перезайдите на сервер lustre, программа должна работать. С большой вероятностью, она будет работать и на head. Если нет, то запускать программу и ставить задачи с её участием в очередь исполнения нужно будет с lustre.
