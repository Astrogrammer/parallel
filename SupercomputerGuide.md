...
2. Через `module load gcc/7.5.0` в явном виде загрузите `gcc-7.5.0`, чтобы получить расширенный список доступных модулей.
3. Повторно исполните `module avail`. Появились два новых доступных модуля: `openmpi/5.0.8` и `python/3.9.24`.
...
5. При желании можно загрузить модуль `python/3.9.24`. Вместе с ним автоматически загрузятся зависимости, среди которых `sqlite/3.50.4` и `openssl/3.4.3`.
...
```
sbatch -c 240 (--partition=mic) start_mpi.sh 240 (arg2) (...)
```
Здесь  
...
    `--partition` - ключ задания очереди  
    `mic` - название очереди (используется по умолчанию в MIC-сегменте)  
    `start_mpi.sh` - название скрипта, который исполняет программу  
    `240` - аргумент скрипта `start_mpi.sh`, определяющий количество процессов в MPI-программе  
