...
### <a id="error_entry">Могу ли я самостоятельно устанавливать программы на кластер?</a>
...
2. Создайте папку для хранения исходных кодов (к примеру, `/lustre/home/ivanov_ii/tar`). Перейдите в неё и скачайте при помощи команды `wget` архив с программой в формате `.tar.gz` или аналогичном.
3. Распакуйте архив при помощи команды `tar xvfz` или `tar xvf`.
4. Перейдите в папку с исходными кодами программы и далее следуйте инструкциям по установке данной конкретной программы. Они могут содержаться в `README`, `INSTALL` или на официальном сайте. В общем виде, установка выглядит следующим образом.
5. Исполните файл `./autogen.sh`, если такой присутствует. Если его нет, выполните `autoreconf -i` для перенстройки конфигурационных файлов под текущую ОС. Если команда не срабатывает, этот шаг можно пропустить.
6. Сконфигурируйте программу. Для этого сначала выполните `./configure -h` для отображения полного списка доступных ключей. Выберите те из них, которые кажутся вам наиболее подходящими. Обязателен к использованию `--prefix=/lustre/home/ivanov_ii/...`, который обеспечит установку программы в вашу личную папку, для чего вам не потребуются рут-права.
